...
import { createJob, runBackfillCoversJob } from "@/services/admin-jobs";
import { createLogger } from "@/lib/logger";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

const log = createLogger("api/admin/cache/backfill-covers");
...
export async function POST(request: Request) {
  ...
  try {
    const jobId = await createJob("backfill_covers", auth.user!.id);
    log.info("backfill_covers job created", { jobId, limit, adminId: auth.user!.id });
    void runBackfillCoversJob(jobId, limit);
