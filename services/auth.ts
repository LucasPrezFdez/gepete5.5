...
async function getAvailableUsername(baseUsername: string) {
  ...
  for (let suffix = 0; suffix < 50; suffix += 1) {
    const candidate = suffix === 0 ? baseUsername : `${baseUsername}-${suffix + 1}`;
    const existing = await sql.query("select id from app_users where username = $1 limit 1", [candidate]) as { id: string }[];
...
function getAuthSecret() {
  return process.env.AUTH_SECRET || process.env.DATABASE_URL || "gameindex-local-dev-secret";
}
