...
export default async function AdminUserDetailPage({ params }: { params: Params }) {
  ...
  const userRows = (await sql.query(
    `select id, email, username, display_name, created_at, updated_at, is_admin,
            banned_at, banned_until, banned_reason
  ...
  const banned =
    user.banned_at && (!user.banned_until || new Date(user.banned_until).getTime() > Date.now());

  const [reviewsCount, listsCount, ratingsCount, reportsAgainst, reportsBy, recentReviews, recentLists] = await Promise.all([
  ...
  return (
    <>
      <AdminPageHeader
        ...
        crumbs={[{ label: "Admin", href: "/admin" }, { label: "Usuarios", href: "/admin/users" }, { label: `@${user.username}` }]}
        actions={
          <UserAdminActions
            userId={user.id}
            ...
            publicProfileHref={`/users/${encodeURIComponent(user.username)}`}
          />
        }
      />
      ...
      <section className="mb-6 flex flex-wrap items-center gap-2">
        ...
        {banned && (
          <span className="rounded-md bg-danger/15 px-2 py-1 text-[11px] font-bold uppercase tracking-wider text-danger">
            Baneado{user.banned_until ? ` hasta ${new Date(user.banned_until).toLocaleString("es-ES")}` : " (permanente)"}
          </span>
        )}
        <span className="rounded-md bg-white/[0.04] px-2 py-1 text-[11px] font-mono text-muted">
          Alta: {new Date(user.created_at).toLocaleDateString("es-ES", { day: "2-digit", month: "short", year: "numeric" })}
        </span>
      </section>

      {banned && user.banned_reason && (
        <p className="mb-4 rounded-xl border border-danger/30 bg-danger/10 p-3 text-[13px] text-foreground">
          <strong className="font-semibold text-danger">Motivo de la suspensión:</strong>{" "}
          <span className="text-muted">{user.banned_reason}</span>
        </p>
      )}

      <p className="mb-6 rounded-xl border border-white/[0.06] bg-white/[0.02] p-3 text-[12.5px] text-muted">
        El estado de administrador se controla únicamente mediante la variable{" "}
        <code className="rounded bg-white/[0.06] px-1 py-0.5 font-mono">ADMIN_EMAILS</code>. No hay UI para
        promover ni degradar admins desde aquí.
      </p>
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <StatCard label="Reportes contra" value={counts.reportsReceived} tone={counts.reportsReceived > 0 ? "warn" : "default"} />
        <StatCard label="Reportes hechos" value={counts.reportsSubmitted} />
      </section>

      <section className="mt-8 grid gap-6 lg:grid-cols-2">
        <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5">
          <h2 className="mb-3 text-base font-semibold tracking-tight">Reseñas recientes</h2>
          {(recentReviews as any[]).length === 0 ? (
            ...
            <ul className="space-y-1.5">
              {(recentReviews as any[]).map((row) => (
                ...
                >
                  <Link
                    href={row.game_slug ? `/games/${row.game_slug}/reviews` : "#"}
                    className="flex min-w-0 flex-1 items-center gap-2 hover:text-foreground"
                  >
                    <span className="truncate font-medium">{row.title}</span>
                    {row.game_title && <span className="truncate text-[11px] text-muted">— {row.game_title}</span>}
                  </Link>
                  <div className="flex shrink-0 items-center gap-2 text-[11px]">
                    <span className="rounded bg-white/[0.06] px-1.5 py-0.5 font-mono">{row.score}/10</span>
                    {row.hidden_at && (
        ...
        <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5">
          <h2 className="mb-3 text-base font-semibold tracking-tight">Listas recientes</h2>
